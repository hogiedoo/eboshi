...
class LineItem < ActiveRecord::Base
  ...
  named_scope :on_week,  lambda { |date| { :conditions => "WEEK(CONVERT_TZ(`start`,'+00:00','#{Time.zone.formatted_offset}'),0)=WEEK('#{date.to_date}',0)" } }
  ...
  named_scope :on_year,  lambda { |date| { :conditions => "YEAR(CONVERT_TZ(`start`,'+00:00','-08:00'))=YEAR('#{date.to_date}')" } }
